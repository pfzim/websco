...
Для повышения отзывчивости консоли, списки папок, ранбуков/плейбуков и их параметров
загружаются в локальную БД. А для уменьшения количества LDAP запросов при
проверке прав доступа можно подключить memcached.

В отличии от AWX в WebSCO плейбуки можно распределить по папкам, что более удобно.

Доступ к ранбукам и плейбукам регулируется по папкам через группы AD. При настройке нужно указывать DN групп. Нет наследования, но есть возможность скопировать права на все вложенные папки.
...
Для того, чтобы поля ранбуков отображались как выпадающий список, чек-бокс или календарь для ввода даты, к названиям полей в конце через / слэш нужно добавить ключи:
 ...
 - w - скрытое поле будет содержать логин пользователя, который запусил ранбук в WebSCO (who run) (в плейбуках для этого можно использовать переменную who_websco)
