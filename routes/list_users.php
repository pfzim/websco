...
function list_users(&$core, $params)
{
	...
	$core->db->select_assoc_ex($users, rpv('SELECT u.`id`, u.`login`, u.`mail`, u.`flags` FROM @users AS u WHERE (u.`flags` & (0x0001 | 0x0002)) = 0x0000 ORDER BY u.`login`'));

	include(TEMPLATES_DIR.'tpl.admin-users.php');
}
